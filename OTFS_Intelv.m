...
%% %%%%%%%%%%%%%%%%%%%%%%% OBJECT GENERATION %%%%%%%%%%%%%%%%%%%%%%% %%
hIntrlver = Interleaver('InterleaverIndicesSource',"Random",...
    'InputLength',nSymbolsPerBlock,...
    'Type',"MatCol",...
    'InterleaverRandomSeed',rand);
...
snrRange = 20;%10:2:30; % in dB
simIterations = 1000 * 20* ones(1,length(snrRange));
...
%         txBits = zeros(size(txBits));  % FOR TEST
        ...
        % for delay-domain interleaver
        txBlocks = hIntrlver.interleave(txBlocks);
...
%         figure(1); clf; plot(rxEqBlocks, '.'); grid on; grid minor; xlim([-1.5 1.5]); ylim([-1.5 1.5]); % hold on; plot(rxIdealEqBlocks, '.'); hold off;
        rxEqBlocks=hIntrlver.deinterleave(rxEqBlocks);
